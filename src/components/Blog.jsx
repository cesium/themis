...
function ShowPaper({ paper }) {
  ...
  return (
    <div className="mb-6 space-y-8 rounded-2xl bg-jordi-black/60 py-14 px-10 text-start shadow-xl shadow-black/5 backdrop-blur">
      ...
      <p className="mt-1 font-mono text-sm text-jordi-beige-500">
        {paper.course}
      </p>
      <p className="mt-1 tracking-tight text-jordi-orange-400">{paper.title}</p>
      <p className={`text-base ${open ? 'text-jordi-beige' : 'hidden'}`}>
        {paper.text}
      </p>
      <p className={`text-base ${open ? 'hidden' : 'text-jordi-beige/70'}`}>
        {paper.summary}
      </p>
      <Button

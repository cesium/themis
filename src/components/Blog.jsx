...
function ShowPaper({ paper }) {
  ...
  return (
    <div className="shadow-secondary/5 mb-6 space-y-8 rounded-2xl bg-jordi-beige/60 py-14 px-10 text-start shadow-xl backdrop-blur">
      <p className="text-xl font-semibold tracking-tight text-jordi-orange-500">
        {paper.author}
      </p>
      <p className="mt-1 font-mono text-sm text-jordi-orange-500">
        {paper.course}
      </p>
      <p className="mt-1 tracking-tight text-jordi-orange-400">{paper.title}</p>
      <p className={`text-base ${open ? 'text-jordi-black' : 'hidden'}`}>
        {paper.text}
      </p>
      <p className={`text-base ${open ? 'hidden' : 'text-jordi-black/70'}`}>
        {paper.summary}
      </p>
      <Button
  ...
  return (
    <div className="relative pt-10 pb-20 sm:py-5">
      <div className="absolute inset-x-0 -top-48 -bottom-14 overflow-hidden bg-white">
        <Image
          className="absolute top-0 left-0 translate-y-[-10%] translate-x-[-55%] opacity-80 blur-md sm:left-1/2 sm:translate-y-[-6%] sm:translate-x-[-98%] lg:translate-x-[-106%] xl:translate-x-[-122%]"
          src={backgroundImage}
          alt=""
          width={918}
          height={1495}
          ...
          unoptimized
        />
        <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-jordi-beige-100" />
